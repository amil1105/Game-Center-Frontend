...
function LobbyPage() {
  ...
  const [notificationHelpDialogOpen, setNotificationHelpDialogOpen] = useState(false);
    ...
    try {
      ...
      try {
        ...
        if (response.data.players && Array.isArray(response.data.players)) {
          ...
          // Eğer playersDetail'de oyuncu bulamadıysak players'dan bilgileri kullan
  ...
  // Geri sayım süresi değiştiğinde, zamanı azalt
  useEffect(() => {
...
function LobbyPage() {
  ...
  useEffect(() => {
    ...
    const loadSoundFile = (path) => {
      try {
        // Tüm olası yolları dene - assetleri de ekleyerek
        const possiblePaths = [
          ...
          `/assets/${path}`,                    // Assets klasöründen
          `${process.env.PUBLIC_URL}/assets/${path}`, // Public assets klasöründen
          `./assets/audio/${path}`,             // Assets/audio klasöründen
          `/public/assets/${path}`,             // Public assets klasöründen           
          `https://assets.game-center.io/sounds/${path}` // CDN üzerinden (örnek)
        ];
        ...
        // Hiçbir yol çalışmadıysa boş bir ses elemanı döndür
        console.error(`Hiçbir ses dosyası yolu çalışmadı: ${path}, boş bir ses elemanı kullanılacak`);
        return new Audio(); // Boş bir ses elemanı
      } catch (e) {
        console.error(`Ses dosyası oluşturma hatası:`, e);
        return new Audio(); // Hata durumunda boş bir ses elemanı
      }
    };
  ...
  const sendNotification = (title, message, icon = '/logo.png') => {
    ...
    // Tarayıcı bildirimi VEYA alternatif yöntemler kullan
    if (notificationsEnabled && notificationPermission === 'granted') {
      ...
      try {
        ...
        // Hata durumunda alternatif olarak sesli bildirim kullan
        useAlternativeNotification(title, message);
      }
    } else {
      // Tarayıcı bildirimi kullanılamadığında alternatif bildirim yöntemleri kullan
      useAlternativeNotification(title, message);
    }
  };

  // Alternatif bildirim yöntemleri için yeni bir yardımcı fonksiyon ekleyelim
  const useAlternativeNotification = (title, message) => {
    console.log('Alternatif bildirim kullanılıyor:', { title, message });
    
    // 1. Ses çal (eğer sesler aktif ise)
    if (soundsEnabled) {
      // Mesaja göre uygun sesi seç
      if (title.toLowerCase().includes('katıl')) {
        playSound('join');
      } else if (title.toLowerCase().includes('hazır')) {
        playSound('ready');
      } else if (title.toLowerCase().includes('oyun') || title.toLowerCase().includes('başl')) {
        playSound('startGame');
      } else {
        playSound('message');
      }
    ...
    // 2. Toast mesajı göster (her durumda)
    ...
    // 3. Sekmeyi yanıp sönmesi için favicon değişikliği eklenebilir
    // Bu özellik daha karmaşık olduğu için şimdilik eklenmedi
    
    console.log('Alternatif bildirim gösterildi');
  };
  ...
  const playSound = (soundType) => {
    ...
    try {
      ...
      if (!sound) {
        console.error(`Ses dosyası bulunamadı: ${soundType}, ses kaynağı oluşturuluyor...`);
        // Hata durumunda sesli uyarıyı taklit et (beep sesi)
        useBeepFallback();
        return;
      }
      ...
      // Ses çalma işlemini dene - içerik kontrolü ekle
      try {
        // MediaElement.readyState değerleri: 
        // 0 = HAVE_NOTHING, 4 = HAVE_ENOUGH_DATA
        if (sound.readyState === 0) {
          console.warn(`Ses dosyası hazır değil: ${soundType}, yüklenme bekleniyor...`);
          // Lokal ses kaynağı desteği ile beep sesi çıkar
          useBeepFallback();
          return;
        }
          ...
          playPromise
            ...
            .catch(err => {
              ...
              if (err.name === 'NotAllowedError') {
                ...
                // Diğer hatalar için beep sesi kullan
                useBeepFallback();
              }
            });
        } else {
          console.warn('Ses çalma işlemi desteklenmiyor, beep sesi kullanılıyor');
          useBeepFallback();
        }
      } catch (error) {
        console.error(`Ses çalma sırasında hata: ${soundType}`, error);
        useBeepFallback();
      }
    } catch (error) {
      console.error(`Genel ses çalma hatası (${soundType}):`, error);
      // Fallback - Web Audio API kullanarak beep sesi çal
      useBeepFallback();
    }
  };

  // Beep sesi çalmak için yardımcı fonksiyon - ses dosyaları çalışmazsa
  const useBeepFallback = () => {
    createBeepSound(); // Bileşen dışında tanımlanmış fonksiyonu çağır
  };

  // Bildirim toggle fonksiyonunu güncelliyorum
  const toggleNotifications = () => {
    ...
    // Bildirimler reddedilmişse ve bildirimler kapalıysa
    if (!notificationsEnabled && notificationPermission === 'denied') {
      // Bildirim yardım modalını aç
      const detectBrowser = () => {
      ...
      setNotificationHelpDialogOpen(true);
      
      // Bildirimler reddedilmişse bile sesli bildirimleri etkinleştir
      ...
      // Kullanıcıya sesli bildirimlerin etkinleştirildiğini bildir
      enqueueSnackbar('Web bildirimleri için izin gerekiyor. Geçici olarak sadece sesli bildirimler etkinleştirildi.', { 
        variant: 'info',
        autoHideDuration: 5000
      });
    } else {
  ...
  // Bildirim yardım dialogunu kapat
  const handleNotificationHelpDialogClose = () => {
    setNotificationHelpDialogOpen(false);
  };
  ...
  const handleCopyShareLink = () => {
    const link = shareLinkType === 'code' ? lobbyCode : window.location.href;
  ...
  const handleShareLink = (platform) => {
    ...
    switch (platform) {
      case 'facebook':
        ...
        break;
      case 'twitter':
        shareUrl = `https://twitter.com/intent/tweet?url=${encodeURIComponent(link)}&text=${encodeURIComponent('Bana katıl ve oynayalım!')}`;
        break;
      case 'whatsapp':
        shareUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(`Bana katıl ve oynayalım! ${link}`)}`;
        break;
      case 'telegram':
        shareUrl = `https://telegram.me/share/url?url=${encodeURIComponent(link)}&text=${encodeURIComponent('Bana katıl ve oynayalım!')}`;
        break;
      case 'discord':
        shareUrl = `https://discord.com/invite/game-center`;
        break;
      case 'instagram':
        shareUrl = `https://www.instagram.com/share?url=${encodeURIComponent(link)}`;
        break;
      case 'link':
        shareUrl = link;
        break;
      case 'email':
        shareUrl = `mailto:?subject=${encodeURIComponent('Bana katıl ve oynayalım!')}&body=${encodeURIComponent(`Bana katıl ve oynayalım! ${link}`)}`;
        break;
      default:
        console.error('Geçersiz paylaşım platformu:', platform);
        return;
    }
  ...
  // Atılma durumu diyaloğunu kapat
  ...
  const joinLobby = async () => {
    try {
      const response = await axiosInstance.post('/lobby/join', {
        lobbyCode,
        userId: user.id
      });
      ...
      if (response.data.success) {
        setHasJoinedLobby(true);
        enqueueSnackbar('Lobiye katıldınız', {
          variant: 'success',
          autoHideDuration: 3000
        });
      } else {
  ...
  // Lobi oluşturma işlemi
  const createLobby = async () => {
    try {
      const response = await axiosInstance.post('/lobby/create', {
        userId: user.id
      });
      ...
      if (response.data.success) {
        ...
        setHasJoinedLobby(true);
        enqueueSnackbar('Lobi oluşturuldu', {
          variant: 'success',
          autoHideDuration: 3000
        });
      } else {
        enqueueSnackbar('Lobi oluşturulurken bir hata oluştu', {
          variant: 'error',
          autoHideDuration: 5000
        });
      }
  ...
  // Lobi verilerini getirme işlemi
  const fetchLobbyData = async (showLoading = true) => {
    if (showLoading) setLoading(true);
    
    try {
      const response = await axiosInstance.get(`/lobby/${lobbyCode}`);
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Lobi verileri alınamadı');
      }
    } catch (error) {
      console.error('Lobi verileri alınamadı:', error);
      setError('Lobi verileri alınamadı');
    } finally {
      setLoading(false);
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        enqueueSnackbar('Mesaj gönderilemedi', {
          variant: 'error',
          autoHideDuration: 5000
        });
      }
  ...
  // Hazır durumu değiştirme işlemi
  const toggleReadyStatus = async () => {
    // Lobi ID kontrolü
    if (!lobby || !lobby._id) {
      enqueueSnackbar('Lobi ID bulunamadı', {
        variant: 'error',
        autoHideDuration: 5000
      });
      return;
    ...
    try {
      const response = await axiosInstance.post('/lobby/toggle-ready', {
        lobbyId: lobby._id,
        userId: user.id
      });
      ...
      if (response.data.success) {
        ...
        enqueueSnackbar('Hazır durumu değiştirilemedi', {
          variant: 'error',
          autoHideDuration: 5000
        });
      }
    } catch (error) {
      console.error('Hazır durumu değiştirme hatası:', error);
      enqueueSnackbar('Hazır durumu değiştirilemedi', {
        variant: 'error',
        autoHideDuration: 5000
      });
  ...
  const startGame = async () => {
    try {
      ...
      if (response.data.success) {
        enqueueSnackbar('Oyun başlatıldı', {
          variant: 'success',
          autoHideDuration: 3000
        });
      } else {
        enqueueSnackbar('Oyun başlatılamadı', {
          variant: 'error',
          autoHideDuration: 5000
        });
      }
  ...
  // Oyuncuyu atma işlemi
  const kickPlayer = async (playerId) => {
    try {
      const response = await axiosInstance.post('/lobby/kick', {
        lobbyCode,
        playerId
      });
      
      if (response.data.success) {
        enqueueSnackbar('Oyuncu atıldı', {
          variant: 'success',
          autoHideDuration: 3000
        });
        fetchLobbyData();
      } else {
        enqueueSnackbar('Oyuncu atılamadı', {
          variant: 'error',
          autoHideDuration: 5000
        });
      }
  ...
  const deleteLobby = async () => {
    try {
      const response = await axiosInstance.delete(`/lobby/${lobbyCode}`);
      
      if (response.data.success) {
        enqueueSnackbar('Lobi silindi', {
          variant: 'success',
          autoHideDuration: 3000
        });
        navigate('/home');
      } else {
        enqueueSnackbar('Lobi silinemedi', {
          variant: 'error',
          autoHideDuration: 5000
        });
      }
  ...
  const addBot = async () => {
    try {
      const response = await axiosInstance.post('/bot/add', {
        lobbyCode
      });
      
      if (response.data.success) {
        enqueueSnackbar('Bot eklendi', {
          variant: 'success',
          autoHideDuration: 3000
        });
        fetchLobbyData();
      } else {
        enqueueSnackbar('Bot eklenemedi', {
          variant: 'error',
          autoHideDuration: 5000
        });
      }
...
function LobbyPage() {
  ...
  const useBeepFallback = () => {
    try {
      ...
      oscillator.frequency.value = 440; // A4 nota frekansı (Hz)
      gainNode.gain.value = 0.1; // Ses seviyesi (0-1)
      
      // Beep sesini çal (200ms)
      oscillator.start();
      setTimeout(() => {
        oscillator.stop();
      }, 200);
      
      console.log('Fallback beep sesi çalındı');
    } catch (e) {
      console.error('Fallback beep sesi çalınamadı:', e);
    }
  };
...
function LobbyPage() {
  ...
  const toggleNotifications = () => {
    ...
    if (!notificationsEnabled && notificationPermission !== 'granted') {
      ...
      // İzin verilmiş, sadece açıp kapatıyoruz
  ...
  const requestNotificationPermission = async () => {
    ...
    try {
      ...
      // Eğer izin zaten denied ise doğrudan yardım dialogunu göster
  ...
  // Yeni state değişkeni ekliyorum
  const [notificationHelpDialogOpen, setNotificationHelpDialogOpen] = useState(false);
  const [browserType, setBrowserType] = useState('');
  ...
  // Yeni Bildirim Yardım Dialogu bileşeni
  {/* Bildirim izin yardımı dialog */}
  <Dialog
    open={notificationHelpDialogOpen}
    onClose={handleNotificationHelpDialogClose}
    maxWidth="md"
  >
    <DialogTitle>
      <Box display="flex" alignItems="center">
        <NotificationIcon color="primary" sx={{ mr: 1 }} />
        Bildirim İzinlerini Etkinleştirme
      </Box>
    </DialogTitle>
    <DialogContent>
      <DialogContentText component="div">
        <Typography paragraph>
          Tarayıcınız bildirimlere izin vermiyor. Bildirimler, lobide biri katıldığında veya mesaj gönderdiğinde
          sizi haberdar etmek için kullanılır.
        </Typography>
        
        <Typography variant="h6" gutterBottom sx={{ mt: 2 }}>
          {browserType} tarayıcısında bildirimleri etkinleştirmek için:
        </Typography>
        
        {browserType === 'Chrome' && (
          <Box component="ol" sx={{ pl: 2 }}>
            <Typography component="li">Tarayıcının adres çubuğunun sağındaki kilit veya bilgi simgesine tıklayın.</Typography>
            <Typography component="li">Açılan menüden "Site ayarları" seçeneğine tıklayın.</Typography>
            <Typography component="li">"Bildirimler" ayarını bulun ve "İzin ver" olarak değiştirin.</Typography>
            <Typography component="li">Sayfayı yenileyin ve bildirimleri tekrar etkinleştirmeyi deneyin.</Typography>
          </Box>
        )}
        
        {browserType === 'Firefox' && (
          <Box component="ol" sx={{ pl: 2 }}>
            <Typography component="li">Tarayıcının adres çubuğunun solundaki kilit simgesine tıklayın.</Typography>
            <Typography component="li">Açılan panelde "İzinleri düzenle" seçeneğine tıklayın.</Typography>
            <Typography component="li">"Bildirimleri gönder" ayarını "İzin ver" olarak değiştirin.</Typography>
            <Typography component="li">Sayfayı yenileyin ve bildirimleri tekrar etkinleştirmeyi deneyin.</Typography>
          </Box>
        )}
        ...
        {browserType === 'Safari' && (
          <Box component="ol" sx={{ pl: 2 }}>
            <Typography component="li">Safari menüsünden "Tercihler" seçeneğine tıklayın.</Typography>
            <Typography component="li">"Websites" (Web siteleri) sekmesine gidin.</Typography>
            <Typography component="li">Sol menüden "Notifications" (Bildirimler) seçeneğine tıklayın.</Typography>
            <Typography component="li">Bu web sitesini bulun ve izin ayarını "Allow" (İzin ver) olarak değiştirin.</Typography>
            <Typography component="li">Sayfayı yenileyin ve bildirimleri tekrar etkinleştirmeyi deneyin.</Typography>
          </Box>
        )}
        ...
        {browserType === 'Edge' && (
          <Box component="ol" sx={{ pl: 2 }}>
            <Typography component="li">Tarayıcının adres çubuğunun sağındaki kilit veya bilgi simgesine tıklayın.</Typography>
            <Typography component="li">Açılan menüden "Site izinleri" seçeneğine tıklayın.</Typography>
            <Typography component="li">"Bildirimler" ayarını "İzin ver" olarak değiştirin.</Typography>
            <Typography component="li">Sayfayı yenileyin ve bildirimleri tekrar etkinleştirmeyi deneyin.</Typography>
          </Box>
        )}
        ...
        {browserType !== 'Chrome' && browserType !== 'Firefox' && browserType !== 'Safari' && browserType !== 'Edge' && (
          <Box component="ol" sx={{ pl: 2 }}>
            <Typography component="li">Tarayıcı ayarlarınızdan "Gizlilik ve Güvenlik" bölümünü bulun.</Typography>
            <Typography component="li">"Site Ayarları" veya "İzinler" kısmına gidin.</Typography>
            <Typography component="li">Bildirimlerle ilgili ayarları bulun ve bu site için bildirimleri etkinleştirin.</Typography>
            <Typography component="li">Sayfayı yenileyin ve bildirimleri tekrar etkinleştirmeyi deneyin.</Typography>
          </Box>
        )}
        ...
        <Typography variant="subtitle2" color="text.secondary" sx={{ mt: 2 }}>
          Not: Bildirimler etkin olmadığında, yeni mesaj ve katılım bildirimleri için sadece sesli ve ekran içi uyarılar kullanılacaktır.
        </Typography>
      </DialogContentText>
    </DialogContent>
    <DialogActions>
      <Button 
        onClick={() => {
          window.open('https://support.google.com/chrome/answer/3220216?hl=tr', '_blank');
        }}
        color="primary"
      >
        Daha Fazla Bilgi
      </Button>
      <Button onClick={handleNotificationHelpDialogClose} color="primary" variant="contained">
        Anladım
      </Button>
    </DialogActions>
  </Dialog>
